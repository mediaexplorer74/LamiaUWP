            ...
            ClientParameter<T2> param2, ClientParameter<T3> param3)
        {
            ...
            switch (query)
            {
                case ClientQuery.PopulationMemberInventoryResourceAmount:
                    if (inventory.TryGetValue(param3.Get as string, out float value))
                    {
                        result = new QueryResult<float>(value) as QueryResult<T>;
                    }
                    else
                    {
                        result = new QueryResult<float>(0f) as QueryResult<T>;
                    }
                    break;
            }
        }
                    ...
                    foreach(var(i, behaviour) in task.behaviour.Enumerate())
                    {
                        switch (behaviour.method)
                        {
                            case TaskTypeBehaviourMethod.EXTRACT:
                                ...
                                if (!inventory.ContainsKey(behaviour.id))
                                {
                                    inventory.Add(behaviour.id, 0f);
                                }                                
